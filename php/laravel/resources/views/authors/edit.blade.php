    ...
    <form action="{{ route('authors.update', $author) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="bio" id="bio" rows="4"
                      class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-gray-200 @error('bio') border-red-500 @enderror">{{ old('bio', $author->bio) }}</textarea>
            @error('bio')
            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
            @enderror
        </div>
